#!/usr/bin/env bash
# ==============================================================================
# Automated openSUSE Tumbleweed Post-Install Bootstrap for Lenovo ThinkPad X1 Nano Gen 3
...
echo "======================================================================"
echo "    Lenovo ThinkPad X1 Nano - openSUSE Post-Install Bootstrap         "
echo "======================================================================"
...
# 1. Enable Packman Essentials Repository
echo ""
echo "[1/9] Enabling Packman Essentials repository..."
sudo zypper --non-interactive addrepo -cfp 90 \
  https://ftp.gwdg.de/pub/linux/misc/packman/suse/openSUSE_Tumbleweed/Essentials/ packman-essentials 2>/dev/null || true

# 2. Add Vendor Third-Party Repositories (Google Chrome, Sublime Text)
echo ""
...
sudo zypper --non-interactive addrepo -g -f \
  https://download.sublimetext.com/rpm/stable/x86_64/sublime-text.repo 2>/dev/null || true

# Google Chrome
if ! zypper lr google-chrome >/dev/null 2>&1; then
    sudo zypper --non-interactive addrepo -g -f \
      https://dl.google.com/linux/chrome/rpm/stable/x86_64 google-chrome 2>/dev/null || true
fi

sudo zypper --gpg-auto-import-keys refresh || true

# 3. Install Curated openSUSE Zypper Packages
echo ""
echo "[3/9] Installing system & command line packages..."
sudo zypper --non-interactive install -y --force-resolution \
  pattern:devel_basis \
  git zsh curl wget lsof rclone btop nvtop htop tmux \
  cifs-utils snapper btrfsprogs wl-clipboard qimgv feh \
  flatpak dkms kernel-devel kernel-default-devel firewalld thermald podman distrobox \
...
# Configure thermald for 13th-gen Intel CPU
if [ -f /etc/sysconfig/thermald ]; then
    sudo sed -i 's/^THERMALD_OPTS=.*/THERMALD_OPTS="--ignore-cpuid-check"/' /etc/sysconfig/thermald
fi
...
if [ -d "$BACKUP_DIR" ]; then
    ...
    if [ -f "$BACKUP_DIR/.ssh/authorized_keys" ]; then
        ...
        cp -n "$BACKUP_DIR/.ssh/authorized_keys" "$REAL_HOME/.ssh/authorized_keys" 2>/dev/null || true
        chmod 600 "$REAL_HOME/.ssh/authorized_keys"
    fi
    if [ -f "$BACKUP_DIR/.ssh/id_ed25519" ] && [ ! -f "$REAL_HOME/.ssh/id_ed25519_legacy" ]; then
        cp "$BACKUP_DIR/.ssh/id_ed25519" "$REAL_HOME/.ssh/id_ed25519_legacy"
        cp "$BACKUP_DIR/.ssh/id_ed25519.pub" "$REAL_HOME/.ssh/id_ed25519_legacy.pub"
        chmod 600 "$REAL_HOME/.ssh/id_ed25519_legacy"
    fi

    # 2. Git Config
    if [ -f "/mnt/pom/.gitconfig" ] && [ ! -f "$REAL_HOME/.gitconfig" ]; then
        echo "  Restoring Git configuration..."
        cp -a "/mnt/pom/.gitconfig" "$REAL_HOME/.gitconfig"
    fi

    # 3. Rclone Google Drive Config
    if [ -f "$BACKUP_DIR/.config/rclone/rclone.conf" ] && [ ! -f "$REAL_HOME/.config/rclone/rclone.conf" ]; then
        ...
        mkdir -p "$REAL_HOME/.config/rclone" "$REAL_HOME/GoogleDrive"
    ...
    # 4. Antigravity IDE, App & Gemini Configs
    SRC_IDE=""
    ...
    # 7. Flatpak application data (Logseq, Foliate, ZapZap, etc.)
    if [ -f "$BACKUP_DIR/../var_app_settings.tar.zst" ] && [ ! -d "$REAL_HOME/.var/app/com.logseq.Logseq" ]; then
    ...
    # 8. Home Assistant Configurations
...
echo "  - Zypper packages & repos configured                                "
